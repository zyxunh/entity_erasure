...
from diffusers.models.unets.unet_2d_condition import UNet2DConditionOutput, unscale_lora_layers, USE_PEFT_BACKEND, \
    scale_lora_layers, deprecate, BaseOutput
...
class UNet2DConditionModelExtraOut(UNet2DConditionModel):
    @register_to_config
    def __init__(self, sample_size: Optional[int] = None, in_channels: int = 4, out_channels: int = 4,
        ...
        if extra_output_dim is not None:
            conv_out_padding = (conv_out_kernel - 1) // 2
            self.extra_conv_out = nn.Conv2d(block_out_channels[0], out_channels, kernel_size=conv_out_kernel,
                                            padding=conv_out_padding)
        else:
            self.extra_conv_out = None
    ...
    def forward(
        ...
        sample_out = self.conv_out(sample)

        if USE_PEFT_BACKEND:
            # remove `lora_scale` from each PEFT layer
            unscale_lora_layers(self, lora_scale)

        if self.extra_conv_out is not None:
            extra_out = self.extra_conv_out(sample)
        else:
            extra_out = None

        if not return_dict:
            return (sample_out, extra_out)

        return UNet2DConditionOutput(sample=sample_out, extra_out=extra_out)


class CustomUNet2DConditionModel(UNet2DConditionModel):
    def __init__(self, *args, collect_intermediate_feature=False, share_collector=None, **kwargs):
        self.collect_intermediate_feature = collect_intermediate_feature
        self.share_collector = share_collector
    ...
    def forward(
        ...
        for i_downsample_block, downsample_block in enumerate(self.down_blocks):
            ...
            down_block_res_samples += res_samples
            if self.collect_intermediate_feature:
                tmp = self.share_collector.get(f"downsample_block_{i_downsample_block}", [])
                tmp.append(sample.cpu())
                self.share_collector[f"downsample_block_{i_downsample_block}"] = tmp
        ...
        for i, upsample_block in enumerate(self.up_blocks):
            ...
            if self.collect_intermediate_feature:
                tmp = self.share_collector.get(f"upsample_block_{i}", [])
                tmp.append(sample.cpu())
                self.share_collector[f"upsample_block_{i}"] = tmp
