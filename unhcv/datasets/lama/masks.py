...
class DefaultKwargs:
    RandomIrregularMaskGenerator = dict(max_angle=4, max_len=400, max_width=300, min_times=2, max_times=5)
    RandomRectangleMaskGenerator = dict(margin=10, bbox_min_size=250, bbox_max_size=360, min_times=2, max_times=4)

def make_random_irregular_mask(shape, max_angle=4, max_len=60, max_width=20, min_times=0, max_times=10,
                               draw_method=DrawMethod.LINE, variables_ptr=None):
    mask_step_id = variables_ptr.get("mask_step_id", None)
    draw_method = DrawMethod(draw_method)

    height, width = shape
    mask = np.zeros((height, width), np.float32)
    if mask_step_id is not None:
        mask_with_id = np.zeros_like(mask)
    ...
    for i in range(times):
        ...
        for j in range(1 + np.random.randint(5)):
            ...
            if mask_step_id is not None:
                mask_with_id[mask == 1] = mask_step_id
                mask_step_id += 1
                mask[...] = 0
    if mask_step_id is not None:
        variables_ptr["mask_step_id"] = mask_step_id
        mask = mask_with_id
...
class RandomIrregularMaskGenerator:
    def __init__(self, variables_ptr=None, max_angle=4, max_len=60, max_width=20, min_times=1, max_times=10, ramp_kwargs=None,
                 draw_method=DrawMethod.LINE):
        self.variables_ptr = variables_ptr
    ...
    def __call__(self, img, iter_i=None, raw_image=None):
        ...
        img_shape = img if isinstance(img, (list, tuple)) else img.shape[1:]
        return make_random_irregular_mask(img_shape, max_angle=self.max_angle, max_len=cur_max_len,
                                          max_width=cur_max_width, min_times=self.min_times, max_times=cur_max_times,
                                          draw_method=self.draw_method, variables_ptr=self.variables_ptr)


def make_random_rectangle_mask(shape, margin=10, bbox_min_size=30, bbox_max_size=100, min_times=0, max_times=3, variables_ptr=None):
    height, width = shape
    mask = np.zeros((height, width), np.float32)
    ...
    for i in range(times):
        ...
        if mask_step_id is not None:
            mask[start_y:start_y + box_height, start_x:start_x + box_width] = mask_step_id
            mask_step_id += 1
        else:
...
class RandomRectangleMaskGenerator:
    def __init__(self, variables_ptr=None, margin=10, bbox_min_size=30, bbox_max_size=100, min_times=1, max_times=3, ramp_kwargs=None):
        self.variables_ptr = variables_ptr
        self.margin = margin
    ...
    def __call__(self, img, iter_i=None, raw_image=None):
        ...
        return make_random_rectangle_mask(img_shape, margin=self.margin, bbox_min_size=self.bbox_min_size,
                                          ...
                                          max_times=cur_max_times, variables_ptr=self.variables_ptr)
...
class RandomSuperresMaskGenerator:
    ...
    def __call__(self, img, iter_i=None, raw_image=None):
        img_shape = img if isinstance(img, (list, tuple)) else img.shape[1:]
        return make_random_superres_mask(img_shape, **self.kwargs)
...
class MixedMaskGenerator:
    def __init__(self, irregular_proba=1/3, irregular_kwargs=None,
                 ...
                 invert_proba=0, use_mask_step_id=False):
        self.probas = []
        self.gens = []
        self.variables_ptr = {}
        self.use_mask_step_id = use_mask_step_id
        ...
        if irregular_proba > 0:
            ...
            self.gens.append(RandomIrregularMaskGenerator(variables_ptr=self.variables_ptr, **irregular_kwargs))

        if box_proba > 0:
            self.probas.append(box_proba)
            if box_kwargs is None:
                box_kwargs = {}
            self.gens.append(RandomRectangleMaskGenerator(variables_ptr=self.variables_ptr, **box_kwargs))
    ...
    def __call__(self, img, iter_i=None, raw_image=None):
        if self.use_mask_step_id:
            self.variables_ptr["mask_step_id"] = 1
...
if __name__ == "__main__":
    ...
    set_seed(1234)
    mask_generator = get_mask_generator(None, dict(irregular_proba=0, irregular_kwargs=None,
                                                   box_proba=1, box_kwargs=None,
                                                   segm_proba=0., segm_kwargs=None,
                                                   squares_proba=0, squares_kwargs=None,
                                                   superres_proba=0, superres_kwargs=None,
                                                   outpainting_proba=0, outpainting_kwargs=None,
                                                   use_mask_step_id=True))
    for _ in range(100):
        mask = mask_generator(np.zeros([3, 512, 512]))
        mask = gray2color(mask[0])
        cv2.imwrite("/home/tiger/code/unhcv/mask.png", mask)
        breakpoint()
        pass
    breakpoint()
    pass