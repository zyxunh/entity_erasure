...
class MultiPointSampler(BasePointSampler):
    ...
    def __init__(self,
        ...
        self.neg_strategies_prob = [
            negative_bg_prob, negative_other_prob, negative_border_prob
        ]
    ...
    def sample_object(self, sample: DSample):
        if len(sample) == 0:
            ...
            self._neg_masks = {
                strategy: bg_mask
                for strategy in self.neg_strategies
            }
    ...
    def _sample_mask(self, sample: DSample):
        ...
        if len(root_obj_ids) > 1 and random.random() < self.merge_objects_prob:
            max_selected_objects = min(len(root_obj_ids),
                                       self.max_num_merged_objects)
            num_selected_objects = np.random.randint(2,
                                                     max_selected_objects + 1)
        ...
        for obj_id in random_ids:
            obj_gt_mask, obj_pos_segments, obj_neg_segments = self._sample_from_masks_layer(
                obj_id, sample)
            if gt_mask is None:
                gt_mask = obj_gt_mask
            else:
    ...
    def _sample_from_masks_layer(self, obj_id, sample: DSample):
        ...
        if not self.use_hierarchy:
            ...
            gt_mask = sample.get_soft_object_mask(
                obj_id) if self.soft_targets else node_mask
        ...
        gt_mask = sample.get_soft_object_mask(
            selected_node) if self.soft_targets else node_mask
        ...
        if node_info['children']:
            ...
            disabled_children = random.sample(node_info['children'],
            ...
            for child_id in disabled_children:
                ...
                if self.soft_targets:
                    ...
                    gt_mask = np.logical_and(gt_mask,
                                             np.logical_not(child_mask))
    ...
    def _multi_mask_sample_points(self,
                                  selected_masks,
                                  is_negative,
        ...
        if len(each_obj_points) == 1:
            ...
            if len(other_points_union) + len(points) <= self.max_num_points:
                ...
                points.extend(
                    random.sample(other_points_union,
    ...
    def _sample_points(self, mask, is_negative=False, with_first_click=False):
        if is_negative:
            num_points = np.random.choice(np.arange(self.max_num_points + 1),
                                          p=self._neg_probs)
        else:
        ...
        for j in range(num_points):
            ...
            if first_click:
                ...
                point_indices_indx = np.random.choice(np.arange(len(indices)),
                                                      p=indices_probs)
            ...
            if num_indices > 0:
                ...
                click = point_indices[np.random.randint(
                    0, num_indices)].tolist() + [point_indx]
                points.append(click)
    ...
    def _positive_erode(self, mask):
        ...
        eroded_mask = cv2.erode(mask.astype(np.uint8),
                                kernel,
                                iterations=self.positive_erode_iters).astype(
                                    np.bool)
    ...
    def _get_border_mask(self, mask):
        ...
        kernel = np.ones((3, 3), np.uint8)
        expanded_mask = cv2.dilate(mask.astype(np.uint8),
                                   kernel,
                                   iterations=expand_r)
...
def get_point_candidates(obj_mask, k=1.7, full_prob=0.0):
    ...
    dt = cv2.distanceTransform(padded_mask.astype(np.uint8), cv2.DIST_L2,
                               0)[1:-1, 1:-1]
    if k > 0:
...
if __name__ == '__main__':
    multi_point_sampler = MultiPointSampler(24,
                      prob_gamma=0.80,
                      merge_objects_prob=0.15,
                      max_num_merged_objects=2)
    images = np.zeros([256, 256, 3], dtype=np.uint8)
    instance_map = np.zeros([256, 256])
    instance_map[20:40, 20:40] = 1
    sample = DSample(images, instance_map, objects_ids=[1])
    multi_point_sampler.sample_object(sample)
    breakpoint()
    points = np.array(multi_point_sampler.sample_points())
    mask = multi_point_sampler.selected_mask
